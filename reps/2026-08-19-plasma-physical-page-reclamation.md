...
TBD before opening an upstream draft PR. Review should include owners of Ray
Core, Plasma/object spilling, and Linux memory management. This fork draft is
not ready to enter the upstream REP process until concrete reviewers are named.

### Shepherd of the Proposal (should be a senior committer)

TBD before opening an upstream draft PR. A senior Ray committer must agree to
shepherd the proposal before that PR is opened.
...
are self-contained C++/Make programs committed under
[`poc_plasma_reclaim/`](../poc_plasma_reclaim/) in this repository (imported
from measurement-branch commit
[`bce4bfa`](https://github.com/wuxueyang96/enhancements/commit/bce4bfae112a2b3c5b4f44f1c67872da9a48f96d)),
with build and run instructions in
[`poc_plasma_reclaim/README.md`](../poc_plasma_reclaim/README.md). No Ray
checkout, Ray fork, source overlay, patch, or Bazel workspace is required.
...
The single authoritative result set is
[`poc_plasma_reclaim/results/latest/`](../poc_plasma_reclaim/results/latest/):
...
`./run_pocs.sh all ./results/latest`. PoC-A regeneration uses the committed
replay wrapper `results/latest/poc_a_command.sh`, which requires at least
4 GiB of free tmpfs and host-aware active-client selection.

The committed results come from one Linux 5.15 x86_64 host: 32 available
...
skipped by design. The complete aggregate is committed in
`results/latest/poc_a_summary.csv` together with its environment
(`poc_a_environment.txt`) and source hashes (`poc_a_source_sha256.txt`);
per-sample raw data is intentionally regenerated rather than committed, via
`results/latest/poc_a_command.sh`.
...
per-arm output is committed as
`results/latest/poc_b_<scale>_churn_<cadence>.csv/.log` and summarized in
`poc_b_summary.csv`:
...
proposed above. Complete telemetry and the verdict are committed as
`results/latest/poc_c.csv` and `poc_c.log`.
